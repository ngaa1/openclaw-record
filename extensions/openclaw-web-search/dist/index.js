...
// src/search.ts
var ZhipuWebSearchSchema = Type.Object({
  ...
  search_lang: Type.Optional(
    Type.String({
      description: "Language for search results (e.g., 'zh', 'en'). Defaults to auto-detect."
    })
  ),
  freshness: Type.Optional(
    Type.String({
      description: 'Filter by recency: "oneDay", "oneWeek", "oneMonth", "threeMonths", "sixMonths", "oneYear", or "noLimit" (default).'
    })
  )
});
var FRESHNESS_MAP = {
...
function normalizeFreshness(raw) {
  if (!raw) return "noLimit";
  const key = raw.toLowerCase().replace(/[-_\s]/g, "");
  return FRESHNESS_MAP[key] ?? raw;
}
...
`];
  for (let i = 0; i < results.length; i++) {
    ...
    const meta = [];
    if (r.media) meta.push(`Source: ${r.media}`);
    if (r.publish_date) meta.push(`Date: ${r.publish_date}`);
...
function resolveApiKey(config) {
  return config.apiKey || process.env.ZHIPU_API_KEY || process.env.ZAI_API_KEY || void 0;
}
function createZhipuWebSearchTool(config) {
  resolveApiKey(config);
  const baseUrl = (config.baseUrl ?? "https://open.bigmodel.cn").replace(
    /\/+$/,
    ""
  );
  const searchEngine = config.searchEngine ?? "search_std";
  const maxResults = config.maxResults ?? 10;
  const contentSize = config.contentSize ?? "medium";
  return {
    label: "Web Search",
    name: "web_search",
    description: "Search the web for current information. Returns a list of relevant results with titles, URLs, and summaries. Use this when you need up-to-date information or to answer questions about recent events.",
    parameters: ZhipuWebSearchSchema,
    async execute(_toolCallId, params) {
      ...
      if (!resolvedKey) {
        return {
          content: [
            {
              type: "text",
              text: "Error: Zhipu API key not configured. Set apiKey in plugin config or ZHIPU_API_KEY / ZAI_API_KEY environment variable."
            }
          ],
          details: { error: "api_key_missing" }
        };
      }
      const count = Math.min(
        Math.max(Math.round(Number(params.count) || maxResults), 1),
        10
      );
      ...
      const url = `${baseUrl}/api/paas/v4/web_search`;
      let resp;
      try {
        ...
        return {
          content: [
            { type: "text", text: `Error: Failed to reach Zhipu API: ${msg}` }
          ],
          details: { error: msg }
        };
      }
      if (!resp.ok) {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Error: Zhipu API returned ${resp.status}: ${errText}`
            }
          ],
          details: { error: errText, status: resp.status }
        };
      }
      let data;
      try {
        ...
        return {
          content: [
            { type: "text", text: "Error: Failed to parse Zhipu API response." }
          ],
...
var plugin = {
  ...
  name: "Web Search (Zhipu)",
  description: "Web search via Zhipu AI API, accessible from within China",
  register(api) {
    const config = api.pluginConfig ?? {};
    if (config.enabled === false) return;
    api.registerTool(createZhipuWebSearchTool(config), {
      name: "web_search"
    });
  }
};
...
export { createZhipuWebSearchTool, index_default as default };
